...
Para exemplificar esse processo, o Algoritmo 6 apresenta uma função implementada em Rust e preparada para ser exportada para WebAssembly. A função calcula o n-ésimo termo da sequência de Fibonacci e utiliza o atributo \verb|#[wasm_bindgen]| para permitir sua integração com JavaScript.
...
A macro \texttt{\#[wasm\_bindgen]} gera os \emph{bindings} necessários para que a função apresentada no Algoritmo 6 possa ser chamada a partir do JavaScript, conforme demonstrado no Algoritmo 7.
...
Nesse caso, o JavaScript inicializa o módulo WebAssembly e, em seguida, chama a função \texttt{fibonacci}, passando o valor \texttt{30} como parâmetro. O resultado retornado pela função Rust é então exibido no console do navegador.
...
O Algoritmo 8 ilustra esse comportamento por meio de uma variável do tipo \texttt{String}. Ao atingir o final do escopo em que foi declarada, a variável deixa de existir e a memória associada ao seu valor é liberada automaticamente.
...
O exemplo demonstra, portanto, que a liberação da memória ocorre automaticamente ao final do escopo, sem a necessidade de uma chamada explícita para liberar o recurso.

Esse modelo elimina tanto o trabalho manual de liberar memória, que em C frequentemente gera erros, quanto a sobrecarga do coletor de lixo. O resultado é um programa que roda com eficiência comparável à de C e C++, mas sem exigir que o programador gerencie a memória manualmente \cite{bugden2022}.
...
Para ilustrar essa interação entre JavaScript e WebAssembly, o Algoritmo 9 apresenta uma função Rust que recebe uma string como parâmetro e realiza a contagem de suas vogais. O exemplo evidencia o uso de dados textuais na comunicação entre o código JavaScript e o módulo WebAssembly.
